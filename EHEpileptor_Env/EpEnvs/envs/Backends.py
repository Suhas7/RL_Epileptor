...
class JSim:
    ...
    def xhat_1(sim, self) -> float:
        return self.y1 - sim.f1(self) - self.z + self.params['I_rst1']

    def yhat_1(sim, self) -> float:
        return self.params['y0'] - 5 * (self.x1 ** 2) - self.y1

    def zhat(sim, self) -> float:
        return (4 * (self.x1 - self.params['x0']) - self.z) / self.params['tau0']

    def xhat_2(sim, self) -> float:
        return -self.y2 + self.x2 - (self.x2 ** 3) + self.params['I_rst2'] \
               + .002 * sim.g_val - .3 * (self.z - 3.5)

    def yhat_2(sim, self) -> float:
        return (-self.y2 + sim.f2(self)) / self.params['tau2']

    def ghat(sim, self):
        sim.g_val += (-self.params["gamma"] * (sim.g_val - 0.1 * self.x1)) / self.config["Fs"]
